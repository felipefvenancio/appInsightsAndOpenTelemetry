...
{
    public class TraceDemo : IDisposable
    {
        private const string ActivitySourceName = "MyCompany.MyProduct.MyLibrary";
        public readonly ActivitySource s_activitySource = new ActivitySource(ActivitySourceName);
        ...
        public TraceDemo(string connectionString, TokenCredential credential = null)
        {
            ...
            {
                { "service.name", "ConsultaCampanha" },
                { "service.namespace", "Fidelidade" },
        ...
        public void GenerateTraces()
        {
            ...
            using (var activity = s_activitySource.StartActivity("Client Hello", ActivityKind.Client))
            {
                activity?.SetTag("foo", 1);
                activity?.SetTag("baz", new int[] { 1, 2, 3 });
                activity?.SetStatus(ActivityStatusCode.Ok);

                using (var nestedActivity = s_activitySource.StartActivity("Server Say hello", ActivityKind.Server))
                {
                    nestedActivity?.SetTag("bar", "Hello, World!");
                    nestedActivity?.SetStatus(ActivityStatusCode.Ok);

                    using (var nestedActivity2 = s_activitySource.StartActivity("Other operaction", ActivityKind.Server))
                    {
                        nestedActivity2.SetStatus(ActivityStatusCode.Ok, "Okay");
                    }
                }
                try
                {
                    throw new Exception("Client exception before server");
                }
                catch (Exception ex)
                {
            ...
            using (var activity = s_activitySource.StartActivity("ExceptionExample", ActivityKind.Server))
            {
                try
                {
